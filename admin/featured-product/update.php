...
if(isset($_POST['update_product'])){
    ...
    $query = "UPDATE `featured-products` SET name='$phonename', storage='$storage', ram='$ram', processor='$processor' WHERE id='$ids' ";

    $query_run = mysqli_query($con, $query);

    if($query_run){
        header('../featured-product.php');
    }
}
?>
...
<form method="POST" enctype="multipart/form-data">
        ...
        $query = "select * from `featured-products` where id=$ids";
        $query_run = mysqli_query($con, $query);
    ...
    <div class="back-btn" style="position:absolute;top:25px;right:50px;font-size:35px;cursor: pointer;">
        <a href="../featured-product.php">&#10006</a>
    </div>

        <h3 style="margin-top: 30px;text-align:center;">Update Featured Product</h3>

        <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>">

        <label for="username">Phone Name</label>
        <input type="text" style="width: 30%;" placeholder="Phone name" name="phonename" id="phonename" value="<?php echo $product['name']; ?>" required>
