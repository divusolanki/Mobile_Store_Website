...
if(isset($_POST['update_product'])){
    ...
    $query = "UPDATE `apple` SET name='$phonename', storage='$storage', ram='$ram', processor='$processor', launch_date='$launch_date', camera='$camera', display='$display', battery='$battery', price='$price' WHERE id='$ids' ";
...
<form method="POST" enctype="multipart/form-data">
        ...
        $query = "select * from `apple` where id=$ids";
        $query_run = mysqli_query($con, $query);
    ...
    <div class="back-btn" style="position:absolute;top:25px;right:50px;font-size:35px;cursor: pointer;">
        <a href="../apple.php">&#10006</a>
    </div>

        <h3 style="margin-top: 30px;text-align:center;">Update Apple Product</h3>

        <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>">

        <label for="username">Phone Name</label>
        <input type="text" placeholder="Phone name" name="phonename" id="phonename" value="<?php echo $product['name']; ?>" required>
